...
export default function RespondPage({ params }: { params: { token: string } }) {
  ...
  useEffect(() => {
    supabase.from('reports').select('*').eq('respond_token', params.token).single()
      .then(({ data, error }) => {
  ...
  if (invalid) return (
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{maxWidth:400,textAlign:'center'}}>
        ...
        <p style={{fontSize:15,color:C.textSub,lineHeight:1.7}}>This response link is not valid. Contact support@klickifyagency.com if you need assistance.</p>
      </div>
    </div>
  );
  ...
  if (submitted) return (
    <div style={{minHeight:'100vh',background:C.bg,display:'flex',alignItems:'center',justifyContent:'center',padding:24}}>
      <div style={{maxWidth:440,textAlign:'center'}}>
        ...
        <p style={{fontSize:15,color:C.textSub,lineHeight:1.75}}>Your official response is now visible to all Natchez residents. Thank you for your service to Ward {report?.ward_number}.</p>
  ...
  return (
    <div style={{minHeight:'100vh',background:C.bg,paddingBottom:60,fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#1A5EA8,#2D7A4F)',padding:'36px 24px 28px',textAlign:'center'}}>
        <img src="/logo-wtp.png" alt="We The People 39120" style={{height:70,objectFit:'contain',marginBottom:16,display:'block',marginLeft:'auto',marginRight:'auto'}} />
      ...
      <div style={{maxWidth:560,margin:'0 auto',padding:'24px 20px'}}>
        <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,overflow:'hidden',marginBottom:20}}>
          <div style={{padding:'12px 16px',background:C.blueSoft,borderBottom:`1px solid ${C.border}`}}>
            <span style={{fontSize:11,fontWeight:800,color:C.blue,textTransform:'uppercase',letterSpacing:'0.08em'}}>Report Summary</span>
          </div>
          ...
          {report.description && <div style={{padding:'12px 16px'}}><div style={{fontSize:11,fontWeight:800,color:C.textMuted,textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:6}}>Resident Description</div><p style={{fontSize:14,color:C.textSub,lineHeight:1.7,fontStyle:'italic'}}>"{report.description}"</p></div>}
        </div>
        ...
        <div style={{background:'#FEF3C7',border:'1px solid #FCD34D',borderRadius:12,padding:'14px 16px',marginBottom:20,display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <p style={{fontSize:13,color:'#78350F',lineHeight:1.6}}><strong>This link is private and exclusive to you.</strong> Your response will be posted publicly under your name as alderman of Ward {report.ward_number}. Do not share this link.</p>
        </div>
        ...
        <div style={{background:C.white,borderRadius:16,border:`1px solid ${C.border}`,padding:20,marginBottom:16}}>
          ...
          <textarea value={response} onChange={e => { setResponse(e.target.value); setError(''); }}
            placeholder="Example: Thank you for bringing this to my attention. I have contacted Public Works and scheduled an inspection for this week. We expect to resolve this within 10 business days..."
            rows={6}
            style={{width:'100%',padding:'14px 16px',borderRadius:12,border:`1.5px solid ${error ? C.danger : C.border}`,fontSize:15,lineHeight:1.65,resize:'none',outline:'none',color:C.textMain,fontFamily:'inherit',boxSizing:'border-box'}} />
